...
from models import FastWideResNet, FasterWideResNet
...
def run(config, plx_experiment):
    ...
    model = config["model"](width=width, depth=depth,
    ...
    optimizer = optim.SGD(
        [
            ...
            # group 0
            dict([("params", model.group0.parameters())] + opt_kwargs),
            # group 1
            dict([("params", model.group1.parameters())] + opt_kwargs),
            # group 2
            dict([("params", model.group2.parameters())] + opt_kwargs),
            # last bn
            dict([("params", model.bn.parameters())] + opt_kwargs),
            # linear
            dict([("params", model.fc.parameters())] + opt_kwargs),
        ]
    )
